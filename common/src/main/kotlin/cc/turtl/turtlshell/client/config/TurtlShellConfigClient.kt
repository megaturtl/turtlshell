...
import cc.turtl.turtlshell.Platform
import cc.turtl.turtlshell.client.TurtlShellClientEvents
import cc.turtl.turtlshell.client.TurtlShellKeybinds
import cc.turtl.turtlshell.client.config.category.GeneralConfig
import cc.turtl.turtlshell.client.config.custom.KeyAdapter
import cc.turtl.turtlshell.util.modResource
    ...
    companion object {
        private val HANDLER: ConfigClassHandler<TurtlShellConfigClient> =
            ConfigClassHandler.createBuilder(TurtlShellConfigClient::class.java)
                .id(modResource("config"))
                .serializer { config ->
                    GsonConfigSerializerBuilder.create(config)
                        .setPath(Platform.getConfigDir().resolve("config.json"))
                        .appendGsonBuilder {
                            it.setPrettyPrinting()
                                .registerTypeHierarchyAdapter(InputConstants.Key::class.java, KeyAdapter())
                        }
                        .build()
                }
                .build()
        ...
        fun init() {
            HANDLER.load()
            TurtlShellClientEvents.TICK_POST.subscribe {
